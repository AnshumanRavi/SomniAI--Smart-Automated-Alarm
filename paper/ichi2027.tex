...
%% Double-blind: pdfTeX otherwise writes the system username into /Author and
%% the tool chain into /Creator. Guarded so lualatex/xelatex do not choke.
\ifdefined\pdfinfo
  \pdfinfo{%
    /Author ()%
    /Title (Reliability-Targeted Bedtime Planning with Explicit Infeasibility)%
    /Subject ()%
    /Keywords ()%
    /Creator ()%
  }
...
%% ---------------------------------------------------------------
%% AUTHORS. ICHI 2027 Early Bird is DOUBLE-BLIND, so the submitted PDF must
%% carry the anonymous block. Leave \anontrue for the 21 Sep submission.
%%
%% Flip to \anonfalse ONLY for:  camera-ready (if accepted), and the arXiv
%% preprint posted AFTER submitting.
%% ---------------------------------------------------------------
\newif\ifanon
\anontrue                        % <<< \anonfalse for camera-ready / arXiv

%% The real author block deliberately does NOT live in this file. It is in
%% paper/authors.local.tex, which is gitignored, so no name, e-mail or
%% affiliation exists anywhere in the repository -- and therefore not in the
%% anonymised artifact mirror a reviewer browses. Keep it that way.
\ifanon
...
\else
  \IfFileExists{authors.local.tex}{\input{authors.local.tex}}{%
    \PackageError{ichi2027}{authors.local.tex is missing}{%
...
\textbf{Sensitivity.} Table~\ref{tab:sens} sweeps all three choices on PMData
(1{,}785 labelled nights). Across the full grid the positive rate runs from
0.198 to 0.858, and we state that number first because it is the one a reviewer
running our code would find. It decomposes into one robust choice, one nearly
free choice, and one that matters.

Tolerance is robust: halving it to 15 minutes flips 7.7\% of labels and doubling
it to 60 flips 11.4\%, while the positive rate moves only because a wider window
mechanically admits more nights. Day-type bucketing moves the rate least of the
three (spread 0.040) yet flips 16\% of nights against a pooled baseline --- both
facts matter, and reporting only the rate would have hidden the second;
weekend and day-of-week buckets are near-interchangeable (agreement 0.915).

\textbf{One-sidedness is the consequential choice}: a quarter of nights flip
(agreement 0.741) and the positive rate falls to 0.406. We defend it
conceptually above --- oversleeping means waking \emph{late} --- but that is a
conceptual argument, and this is how much rides on it. This sweep covers PMData
only; the LifeSnaps archive was unavailable at the time of writing, so the
larger cohort is not represented here.

\begin{table}[t]
\caption{Target sensitivity on PMData. Agreement is night-by-night against the
paper's setting.}
\label{tab:sens}
\centering
\small
...
\textbf{Complete cases only.} Nights missing any model input or the target are
dropped rather than imputed, since the habit features need trailing history that
the first nights of a record do not have. Table~\ref{tab:data} reports each
cohort as released; the LifeSnaps models are fitted on the 3{,}376 nights from
63 participants that survive this filter, which is the 63 in
Fig.~\ref{fig:splits}.
...
factor of four. Across splits it ranges $-0.077$ to $+0.395$
(Fig.~\ref{fig:splits}), so no single-split figure is reportable; the interval
nonetheless excludes zero, so the model does beat the mean on LifeSnaps. We show
the spread rather than a mean alone because a paper reporting one split from
this distribution could honestly claim anything from failure to a strong result.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig6_across_splits.pdf}
\caption{Metric spread across eight participant-level splits. The width is the
point: with 63 participants, which people land in the test fold moves $R^2$ by
...
(Table~\ref{tab:calib}, Fig.~\ref{fig:calib}), \textbf{ECE is 0.0148 (95\% CI
0.0139--0.0571)}. The interval is wide and right-skewed; the point estimate
...
\caption{Reliability diagram. Points sit close to the diagonal across the
occupied range, which is what makes a refusal threshold meaningful; the bins are
narrow because the model is weakly informative, not because it is uncertain
...
anonymised URL for review.\footnote{%%
%% Anonymised mirror. Verified to contain no author name, e-mail or
%% affiliation. Swap for the real repository URL at camera-ready / arXiv.
